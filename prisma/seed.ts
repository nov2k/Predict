...
async function main() {
  ...
  for (const marketData of markets) {
    const market = await prisma.market.create({
      data: {
        ...marketData,
        videoUrl: (marketData.category === 'crypto' || marketData.category === 'tech') 
    ...
    // Create some initial price history
    ...
    for (let i = 0; i < 6; i++) {
      await prisma.pricePoint.create({
        data: {
          ...
          createdAt: new Date(Date.now() - (6 - i) * 24 * 60 * 60 * 1000) // Last 6 days
        }
      });
    }
  }
